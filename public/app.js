...
async function loadInventoryItems() {
    try {
        ...
        if (response.ok) {
            ...
            const visibleItems = allItems.filter(item => !item.listId || !hiddenLists.includes(item.listId));
...
function createInventoryRow(item) {
    ...
    console.log('Creating row for item:', item.nickname, 'with listId:', item.listId, 'colors:', listColors);
    ...
    // Format dates
    const nextCalDue = item.nextCalibrationDue ? new Date(item.nextCalibrationDue).toLocaleDateString() : 'N/A';
    const maintenanceDue = item.maintenanceDue ? new Date(item.maintenanceDue).toLocaleDateString() : 'N/A';
    const lastMaintenance = item.maintenanceDate ? new Date(item.maintenanceDate).toLocaleDateString() : 'N/A';
    const dateReceived = item.dateReceived ? new Date(item.dateReceived).toLocaleDateString() : 'N/A';
    const dateInService = item.datePlacedInService ? new Date(item.datePlacedInService).toLocaleDateString() : 'N/A';
    const lastCal = item.calibrationDate ? new Date(item.calibrationDate).toLocaleDateString() : 'N/A';
    
    // Format calibration interval
    const calInterval = item.calibrationInterval && item.calibrationIntervalType 
...
function editItem(itemId) {
    ...
    if (!item) { showToast('Item not found in list. Try refreshing.', 'error'); return; }
    showAddItemModal();
    ...
    form.querySelector('#dateReceived').value = item.dateReceived ? item.dateReceived.split('T')[0] : '';
    form.querySelector('#datePlacedInService').value = item.datePlacedInService ? item.datePlacedInService.split('T')[0] : '';
    form.querySelector('#location').value = item.location || '';
    form.querySelector('#calibrationDate').value = item.calibrationDate ? item.calibrationDate.split('T')[0] : '';
    form.querySelector('#nextCalibrationDue').value = item.nextCalibrationDue ? item.nextCalibrationDue.split('T')[0] : '';
...
async function handleAddItem(event) {
    ...
    try {
        ...
        const hasMaintenance = formData.get('maintenanceDate') || formData.get('maintenanceDue') || formData.get('maintenanceInterval');
...
async function deleteListWithItems(listId) {
    try {
        ...
        if (resp.ok) {
            ...
            if (idx >= 0) { hiddenNow.splice(idx,1); saveHiddenLists(hiddenNow); }
            await loadListsIntoSelectors();
            await loadInventoryItems();
...
async function loadListsIntoSelectors(selectAfterName) {
    try {
        ...
        const resp = await fetch('/api/inventory/lists', { headers: { 'Authorization': `Bearer ${authToken}` } });
        if (!resp.ok) {
        ...
        if (listSelect) {
            ...
            listSelect.innerHTML = lists.length === 0 ? '<option value="">No lists available</option>' :
        ...
        if (listOptions) {
            ...
            if (withVirtual.length === 0) {
                ...
                console.log('Rendered list rows:', withVirtual.length, 'children in listOptions:', listOptions.children.length);
...
function setupCalendarGrid(events) {
    ...
    function render(monthDate) {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            const dateStr = inMonth ? new Date(monthDate.getFullYear(), monthDate.getMonth(), dayNum).toISOString().split('T')[0] : null;
            cell.innerHTML = `
            ...
            if (inMonth && dateStr && map.has(dateStr)) {
                ...
                map.get(dateStr).slice(0, 3).forEach(ev => {
                    const a = document.createElement('a');
                    a.href = '#';
                    a.className = 'cal-event';
                    a.textContent = ev.title;
                    a.onclick = (e) => { e.preventDefault(); showItemDetailModal(ev.itemId); };
                    body.appendChild(a);
                });
    ...
    prevBtn && (prevBtn.onclick = () => { calendarCurrent.setMonth(calendarCurrent.getMonth() - 1); render(calendarCurrent); });
    nextBtn && (nextBtn.onclick = () => { calendarCurrent.setMonth(calendarCurrent.getMonth() + 1); render(calendarCurrent); });
}
...
function exportEventsAsIcs(events) {
    ...
    const toUtcDt = (d) => {
        ...
        return dt.getUTCFullYear().toString() + pad(dt.getUTCMonth()+1) + pad(dt.getUTCDate()) + 'T' + pad(dt.getUTCHours()) + pad(dt.getUTCMinutes()) + '00Z';
    };
    const icsLines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//Inventory Manager//EN'
    ];
...
function populateColorGrid(gridId, theme, selectedColor = null) {
    ...
    colors.forEach(color => {
        ...
        option.addEventListener('click', function() {
            ...
            console.log('Found inputs:', { customColorInput: !!customColorInput, customTextColorInput: !!customTextColorInput });
...
function switchColorTheme() {
    ...
    if (customColorInput && customColorInput.value) {
        ...
        selectedColor = { bg: customColorInput.value, text: getContrastTextColor(customColorInput.value) };
        console.log('Maintaining current color selection:', selectedColor);
    }
    
    console.log('Calling populateColorGrid with:', { gridId: 'colorGrid', theme: selectedTheme, selectedColor });
...
function switchEditColorTheme() {
    ...
    console.log('Calling populateColorGrid with:', { gridId: 'editColorGrid', theme: selectedTheme, selectedColor });
...
function setupEditColorSelection() {
    ...
    console.log('Found edit color inputs:', { customColorInput: !!customColorInput, customTextColorInput: !!customTextColorInput });
