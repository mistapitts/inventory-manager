...
router.get('/lists', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    const user = await database_1.database.get('SELECT companyId FROM users WHERE id = ?', [
      userId,
    ]);
    if (!user || !user.companyId)
      return res.status(400).json({ error: 'User not associated with a company' });
    let lists = await database_1.database.all(
...
router.post('/lists', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    await database_1.database.run(
      ...
      [id, user.companyId, name.trim(), listColor, listTextColor],
    );
    const created = await database_1.database.get(
      'SELECT id, name, color, textColor, createdAt, updatedAt FROM lists WHERE id = ?',
      [id],
    );
...
router.put('/lists/:id', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    if (!user || !user.companyId)
      return res.status(400).json({ error: 'User not associated with a company' });
    const list = await database_1.database.get(
      'SELECT id FROM lists WHERE id = ? AND companyId = ?',
    ...
    if (deleteItems === 'true') {
      try {
        const items = await database_1.database.all(
          'SELECT id FROM inventory_items WHERE companyId = ? AND listId = ?',
          [user.companyId, listId],
        );
      ...
      try {
        await database_1.database.run(
          "UPDATE inventory_items SET listId = NULL, updatedAt = datetime('now') WHERE companyId = ? AND listId = ?",
          [user.companyId, listId],
        );
      } catch (updateError) {
...
router.post('/lists/migrate-to-field', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    let fieldList = await database_1.database.get(
      'SELECT id FROM lists WHERE companyId = ? AND name = ?',
      [user.companyId, 'Field'],
    );
    if (!fieldList) {
    ...
    try {
      ...
      if (recordType === 'existing') {
        ...
        if (type === 'calibration') {
          ...
          const currentItem = await database_1.database.get(
            'SELECT id, calibrationDate, nextCalibrationDue FROM inventory_items WHERE id = ?',
            [itemId],
          );
          ...
          try {
            ...
            const updatedItem = await database_1.database.get(
              'SELECT id, calibrationDate, nextCalibrationDue FROM inventory_items WHERE id = ?',
              [itemId],
            );
          ...
          try {
            ...
            // Verify the update worked by checking the item again
            const updatedItem = await database_1.database.get(
              'SELECT id, maintenanceDate, maintenanceDue FROM inventory_items WHERE id = ?',
              [itemId],
            );
...
router.put('/:id', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    const existingItem = await database_1.database.get(
      'SELECT id FROM inventory_items WHERE id = ? AND companyId = ?',
      [itemId, user.companyId],
    );
    if (!existingItem) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      params.push(itemId);
      await database_1.database.run(
        `UPDATE inventory_items SET ${updates.join(', ')}, updatedAt = datetime('now') WHERE id = ?`,
        params,
      );
...
async function resolveItemIdColumn(tableName) {
  try {
    ...
    if (cols?.some((c) => c.name === 'itemId')) return 'itemId';
    if (cols?.some((c) => c.name === 'item_id')) return 'item_id';
    return null;
  } catch {
    return null;
  }
...
router.delete('/:itemId/records/:recordId', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    await database_1.database.run(`DELETE FROM ${table} WHERE id = ? AND itemId = ?`, [
      recordId,
      itemId,
    ]);
...
router.get('/stats/overview', auth_1.authenticateToken, async (req, res) => {
  try {
    ...
    // Get total items
    const totalItems = await database_1.database.get(
    ...
    const outOfService = await database_1.database.get(
      'SELECT COUNT(*) as count FROM inventory_items WHERE companyId = ? AND isOutOfService = 1',
      [user.companyId],
    );
    res.json({
