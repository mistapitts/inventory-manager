...
router.put('/lists/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const list = await database.get('SELECT id FROM lists WHERE id = ? AND companyId = ?', [
      listId,
      user.companyId,
    ]);
...
router.delete('/lists/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    if (deleteItems === 'true') {
      try {
        const items: any[] = await database.all(
    ...
    await database.run('DELETE FROM lists WHERE id = ? AND companyId = ?', [
      listId,
      user.companyId,
    ]);
